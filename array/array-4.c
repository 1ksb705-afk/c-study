...
void f2() 
{
	...
	int bsize = sizeof(b[0]); //0번째 행의 시리즈
	printf("%d\n", bsize); //12바이트

	printf("%d\n", sizeof(b[0][0])); //12바이트 //0행 0번째 원소의 사이즈
	printf("%d\n", sizeof(b) / sizeof(b[0])); //전체 크기/한 행 크기 //24바이트 / 12바이트 = 2
...
void f3() 
{
	...
	// //배열 초기화, 1,2,3,4,5,6
	arrPrint(a); //인수 보면서 함수 호출 a배열을 arrPrint함수를 호출할 때 같이 보낸다.
...
int main(void) //main 함수 1개, 처음 시작할 때 main부터 시작
{
	//f1(); //함수는 ()소괄호가 있어야 한다.
	//f2();
	f3(); //함수호출
